...
public class BTProjectDAO {
	...
	// 프로젝트 아이디로 컨텐츠 수정
	public static boolean updateBTProject(String btProjectId, String btProjectContent) throws SQLException {
		Connection con = null;
		PreparedStatement pstmt = null;
		try {
			con = DBUtil.getConnection();
	...
	public static BTProjectDTO getBTProject(String btProjectId) throws SQLException {
		...
		try {
			...
			if (rset.next()) {
				blood_transfusionUser = new BTProjectDTO(rset.getString("bt_project_name"), rset.getString("bt_project_id"), rset.getString("donor_id"),
						rset.getString("recipient_id"), rset.getString("bt_content"));
			}
		} finally {
